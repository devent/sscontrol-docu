...
\item[\TheDistribution{ubuntu}] Ubuntu 14.04
\end{inparaitem}
the value is set to \qcode{/etc/init.d/maradns}.
...
\TheProperty*[dns!maradns!maradns!maradns\_restart\_services]{maradns\_restart\_services \Arg{services}}

The list of \Arg{services} that are needed to be restarted after the 
configuration was deployed to the server. The with \qcode{maradns\_} prefixed 
...
%% restart_flags
\TheProperty[dns:maradns:restart_flags]{restart\_flags}
\TheProperty*[dns!maradns!restart\_flags]{restart\_flags \Arg{flags}}
\TheProperty*[dns!maradns!maradns!maradns\_restart\_flags]{maradns\_restart\_flags \Arg{flags}}

The \Arg{flags} for the restart command. The with \qcode{maradns\_} prefixed 
property have precendence over the nonprefixed property.
For the distributions
\begin{inparaitem}
\item[\TheDistribution{ubuntu}] Ubuntu 12.04,
...
\TheProperty[dns:maradns:default_binding_address]{default\_\\binding\_\\address}
\TheProperty*[dns!maradns!default\_binding\_address]{default\_binding\_address \Arg{address}}

Sets the default binding \Arg{address}. For the distributions
\begin{inparaitem}
\item[\TheDistribution{ubuntu}] Ubuntu 12.04,
\item[\TheDistribution{ubuntu}] Ubuntu 14.04
\end{inparaitem}
