...
on which the dns server should listen to connections. Set to \Arg{loopback}
...
\TheStatement[dns:a_record]{a\_record}
\TheStatement*[dns!a\_record]{a\_record \Arg{name} [, \Arg{address}] [, \{ ttl \}]}

Adds a new A-record with the specified \Arg{name} and \Arg{address}.

\TheStatement[dns:mx_record]{mx\_record}
\TheStatement*[dns!mx\_record]{mx\_record \Arg{name} [, \Arg{address}] [, \{ ttl priority \}]}

Adds a new MX-record with the specified name. If the \Arg{address}
of the record is specified then a new A-record will be created that maps this 
name to the specified address.

\TheStatement[dns:priority]{priority}
\TheStatement*[dns!priority]{priority \Arg{priority}}

Sets the priority for the MX-record. The default priority is 10.

\TheStatement[dns:cname_record]{cname\_record}
\TheStatement*[dns!cname\_record]{cname\_record \Arg{name} \Arg{alias} [, \{ ttl \}]}
